...
pumpRate = getPumpRate(
    nai="(na[cyt]/vol_ratio[cyt])",
    nao="(na[ecs]/vol_ratio[ecs])",
    ki="(kk[cyt]/vol_ratio[cyt])",
    ko="(kk[ecs]/vol_ratio[ecs])",
    ADP="(ADP[cyt]/vol_ratio[cyt])",
    ATP="(ATP[cyt]/vol_ratio[cyt])",
    Pos="(Pos[cyt]/vol_ratio[cyt])",
    **cfg.NaKPump,
)  # /um**2/ms
pumpRate = f"p_max * ({pumpRate})"  # molecules/um**2/ms
...
Vatp += (
    f"((ADP[cyt]/vol_ratio[cyt])/(ADP[cyt]/vol_ratio[cyt] + {cfg.KmADP_synthase})) * "
)
Vatp += (
    f"((Pos[cyt]/vol_ratio[cyt])/(Pos[cyt]/vol_ratio[cyt] + {cfg.KmPi_synthase})) * "
)
Vatp += f"({o2cyt}/({cfg.Ko2} + {o2cyt})) * "
Vatp += f"({cfg.KiATP_synthase}/({cfg.KiATP_synthase} + ATP[cyt]/vol_ratio[cyt]))"
...
min_leak = initEval(f"{cfg.kleakMin}*p_max*{scale}*(v_initial - {ek})/((2*{pumpRate}))")
pmin = initEval(min_pmax)
if constants["p_max"] < pmin + min_leak:
    print("Pump current is too low to balance K+ currents with gkbar_l {cfg.kleakMin}")
...
# ADP is the total ADP (D2 is MgADP -- used by the pump, D1 is free ADP)
AKb = f"{1/cfg.AK['KMg']} + ADP[cyt]/vol_ratio[cyt] + {cfg.Mg}/vol_ratio[cyt]"
D2 = f"((({AKb} - (({AKb})**2 - ADP[cyt]/vol_ratio[cyt] * {4*cfg.Mg}/vol_ratio[cyt])**0.5)/2)/vol_ratio[cyt])"
D1 = f"(ADP[cyt]/vol_ratio[cyt] - ({D2}))"

# forward rate
MTdivKTKM = (
    f"(AMP[cyt] * ATP[cyt]/(vol_ratio[cyt]**2*({cfg.AK['KmAMP'] * cfg.AK['KmMgATP']})))"
)
fnum = f"({cfg.AK['kp2']*cfg.AK['km1']} * {MTdivKTKM})"
fden = f"({cfg.AK['km1']}*(1 + {1/cfg.AK['KmAMP'] + 1/cfg.AK['KiAMP']}*(AMP[cyt]/vol_ratio[cyt]) + (ATP[cyt]/vol_ratio[cyt])/{cfg.AK['KmMgATP']} + {MTdivKTKM}) + {cfg.AK['kp1'] + cfg.AK['kp2']} * {MTdivKTKM})"
...
gPmaxScale *= 4 * np.pi * cfg.somaR**2 / (avo * 1e-18 * cfg.dx**3 * cfg.alpha_ecs)
gliapump = f"{gPmaxScale} * p_max * ({gliaPumpRate})"  # mM/ms
...
reactions["ATPRestore"] = {
    "reactant": f"5*ADP[cyt] + 5*Pos[cyt] + oxygen[cyt]",
    "product": f"5*ATP[cyt]",
    "rate_f": Vatp,
    "custom_dynamics": True,
}

        ...
        if (c % 2) == 0:
            if c == 2 and r == 0:
                netParams.connParams[str(L[c])+'->'+str(L[r])] = { 
                    ...
                    'weight':'2e-8*max(0, weightMin +normal(0,dweight*weightMin))', # synaptic weight
                ...
                netParams.connParams[str(L[c])+'->'+str(L[r])] = { 
                    ...
                    'postConds': {'pop': L[r]},                        # conditions of postsyn cells
                    'divergence': cfg.ScaleFactor*(np.log(1.-C[r][c])/np.log(1. -1./(N_Full[r]*N_Full[c])) ) /N_Full[c],
                    'weight':'1e-8*max(0, weightMin +normal(0,dweight*weightMin))', # synaptic weight
                    'delay':'max(0.1, delayMin_e +normal(0,ddelay*delayMin_e))',  # transmission delay (ms)
            ...
            netParams.connParams[str(L[c])+'->'+str(L[r])] = { 
                ...
                'postConds': {'pop': L[r]},                        # conditions of postsyn cells
                'divergence': cfg.ScaleFactor*(np.log(1.-C[r][c])/np.log(1. -1./(N_Full[r]*N_Full[c])) ) /N_Full[c],
                'weight':'-4e-8*max(0, weightMin +normal(0,dweight*weightMin))', # synaptic weight
...
# v0.3 - scaled original connections by 1e-8
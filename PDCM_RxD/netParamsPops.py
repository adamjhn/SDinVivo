...
kcc2 = {pop: f"(ukcc2_{pop} * rxd.rxdmath.log((kk[cyt_{pop}] * cl[cyt_{pop}] * vol_ratio[cyt_{pop}]**2) / (kk[ecs] * cl[ecs] * vol_ratio[ecs]**2)))" for pop in L}

# Nerst equation - reversal potentials
escale = 1e3 * h.R * (273.15 + cfg.hParams["celsius"]) / h.FARADAY

ena = {pop: f"{escale} * rxd.rxdmath.log(na[ecs]*vol_ratio[cyt_{pop}]/(na[cyt_{pop}]*vol_ratio[ecs]))" for pop in L}
ek = {pop: f"{escale} * rxd.rxdmath.log(kk[ecs]*vol_ratio[cyt_{pop}]/(kk[cyt_{pop}]*vol_ratio[ecs]))" for pop in L}
ecl = {pop: f"{escale} * rxd.rxdmath.log(cl[cyt_{pop}]*vol_ratio[ecs]/(cl[ecs]*vol_ratio[cyt_{pop}]))" for pop in L}
...
osm = {pop: f"(1.1029 - 0.1029*rxd.rxdmath.exp( ( (na[ecs] + kk[ecs] + cl[ecs] + 18.0)/vol_ratio[ecs] - (na[cyt_{pop}] + kk[cyt_{pop}] + cl[cyt_{pop}] + 132.0)/vol_ratio[cyt_{pop}])/20.0))" for pop in L}
...
for pop in L:
    o2switch[pop] = f"((1.0 + rxd.rxdmath.tanh(1e5 * (%s - 5e-4))) / 2.0)" % (o2cyt)
...
for pop in L:
    ...
    Vatp[pop] += f"((ADP[cyt_{pop}]/vol_ratio[cyt_{pop}])/(ADP[cyt_{pop}]/vol_ratio[cyt_{pop}] + {cfg.KmADP_synthase})) * "
    Vatp[pop] += f"((Pos[cyt_{pop}]/vol_ratio[cyt_{pop}])/(Pos[cyt_{pop}]/vol_ratio[cyt_{pop}] + {cfg.KmPi_synthase})) * "
    Vatp[pop] += f"({o2cyt[pop]}/({cfg.Ko2} + {o2cyt[pop]})) * "
    Vatp[pop] += f"({cfg.KiATP_synthase}/({cfg.KiATP_synthase} + ATP[cyt_{pop}]/vol_ratio[cyt_{pop}]))"
...
for pop in L:
    ...
    min_pmax = f"p_max_{pop} * ({volume_scale}*({nkcc1[pop]} + {kcc2[pop]}) + {gk[pop]} * (v_initial - {ek[pop]}))/(2*{pumpRate[pop]})"
    min_leak = initEval(f"{cfg.kleakMin}*p_max_{pop}*{scale}*(v_initial - {ek[pop]})/((2*{pumpRate[pop]}))")
    pmin = initEval(min_pmax)
    if constants[f"p_max_{pop}"] < pmin + min_leak:
        print(f"Pump current is too low in {pop} to balance K+ currents with gkbar_l {cfg.kleakMin}")
...
ADKf, ADKb = {}, {}
for pop in L:
    pr = initEval(pumpRate[pop]) * Prescale  # cyt mM/ms ATP->ADP + Pos
    ...
    VBasalATP[pop] = pr / 2
    Vmax_ATPsyth = (1.5 * pr) / (5 * initEval(Vatp[pop]))
    print(f"Vmax_ATPsynthase = {Vmax_ATPsyth:.6e} mM/ms")
    Vatp_rate = 5 * Vmax_ATPsyth * initEval(Vatp[pop])
    ...
    # forward rate
    MTdivKTKM = (
        f"(AMP[cyt_{pop}] * ATP[cyt_{pop}]/(vol_ratio[cyt_{pop}]**2*({cfg.AK['KmAMP'] * cfg.AK['KmMgATP']})))"
    )
    ...
    clbalance = f"(-(2*{nkcc1[pop]} + {kcc2[pop]}) * {volume_scale})/({ecl[pop]} - v_initial)"
    kbalance = f"({gk[pop]} * (v_initial - {ek[pop]}) + ({volume_scale} * ({nkcc1[pop]} + {kcc2[pop]})  -2.0 * {pumpRate[pop]}))  / ({ek[pop]} - v_initial)"
    nabalance = f"({gna[pop]} * (v_initial - {ena[pop]}) + ({nkcc1[pop]}*{volume_scale} + 3.0 * {pumpRate[pop]})) / ({ena[pop]} - v_initial)"
...
gPmaxScale = {pop: (
        ...
        * initEval(pumpRate[pop])
        / initEval(gliaPumpRate)
...
g_glia = f"g_gliamax / (1.0 + rxd.rxdmath.exp(-(({o2ecs})/vol_ratio[ecs] - 2.5)/0.2))"
...
for pop in L:
    ...
    regions[f"mem_{pop}"] = {"cells": [pop], "secs": "all", "nrn_region": None, "geometry": "membrane"}
...
netParams.rxdParams["states"] = {
    ...
    "Pos": {"regions": [f"cyt_{pop}" for pop in L], "initial": constants["Pos_initial"], "name": "Pos"},
}
...
for pop in L:
    mcReactions[f"vol_dyn_{pop}"] = {
        ...
        "product": "dump[ecs]",
        "rate_f": f"-1 * (%s) * vtau * ((%s) - vol_ratio[cyt_{pop}])" % (scalei, osm[pop]),
        "membrane": f"mem_{pop}",
    ...
    mcReactions[f"vol_dyn_ecs_{pop}"] = {
        ...
        "rate_f": f"-1 * (%s) * vtau * ((%s) - vol_ratio[cyt_{pop}])" % (scaleo, osm[pop]),
        "membrane": f"mem_{pop}",
        "custom_dynamics": True,
        "scale_by_area": False,
    }
    ...
    mcReactions[f"pump_current_others_{pop}"] = {
    ...
    mcReactions[f"O2Flux_{pop}"] = {
        ...
        "rate_f": f"{420*avo/1e18} *({o2ecs} - {o2cyt[pop]})",
        "membrane": f"mem_{pop}",
        "custom_dynamics": True,
        "membrane_flux": False,
    }
...
rates["n_gate"] = {
    "species": "ngate",
    "regions": ["mem_{pop}" for pop in L],
    "rate": "((%s) * (1.0 - ngate)) - ((%s) * ngate)" % (alpha_n, beta_n),
}

rates["PRestore"] = {
    "species": f"Pos",
    "regions": ["cyt_{pop}" for pop in L],
